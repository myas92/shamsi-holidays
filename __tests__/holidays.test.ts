...
describe("test shmasi-holidays functions", () => {
  ...
  it("isHolidaysOffline: should return status of date for 1400/01/02", async () => {
    const statusHoliday = await isHolidaysOffline('1400/01/02');
    expect(statusHoliday).toBe(true);
  });

  it("isHolidaysOffline: should throw Error of date for 1396/01/02", async () => {
    let statusHoliday = isHolidaysOffline('1396/01/02');
